...
def write_to_file(filename, myname, myage, major):
    ...
    outfile.write("My name is "+ myname + " \n"+"My agee is "+myage+" \n"+"I am majoring in "+major)    # write out the age and major in two lines
    # close the file
    outfile.close()
    infile = open(filename)
...
def name_phone(csv_filename):
    ...
    while True:
        ...
        if nextname == [""]:
            break       
        nextname.append(input("Enter a friend's no, press return to end: "))# break jumps out of the loop
        print(nextname) 
        csv.writer(f).writerow(nextname)
    f.close()
